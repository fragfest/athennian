...
/*
 * COMMENTS
 * .eslintrc file was added to follow some standard rules including ES6 compatibility.
 * New function is now async thanks to a mock lyrics fetching Promise.
 * A test.js file was added to check that refactoring did not alter the original output.
 * A simpler map call replaces the for loop and safer const variables are used.
 * The lyricsObj returned from the Promise is safely checked. This may be incorrect since the original function would have errored.
 * refrainRepeat was not used, just like in the original function. Since we are refactoring, it is safer assuming it remains unrequired.
*/

// Mock async call to get lyrics object
const getLyricsObjAsync = () => Promise.resolve(lyrics);

const momsSpagettiAsync = () => {
    return getLyricsObjAsync().then(lyricsObj => {
        const lyrics = lyricsObj || {};
        const intro = lyrics.intro || "";
        const ending = lyrics.ending || "";
        const refrain = lyrics.refrain || "";
        const choruses = lyrics.choruses || [];
        const getChorusStr = chorusObj => (chorusObj || {}).chorus || "";

        const chorusWithRefrain = chorusObj => getChorusStr(chorusObj) + refrain + refrain;
        const chorusRefrain = choruses
            .map(chorusWithRefrain)
            .join("");

        return intro + chorusRefrain + ending;
    }).catch(err => {
        throw new Error(err);
    });
};

// PROVIDE ACCESS FOR TEST.JS
module.exports = {
    momsSpagetti: () => momsSpagetti(lyrics),
    momsSpagettiAsync: () => momsSpagettiAsync(),
};
